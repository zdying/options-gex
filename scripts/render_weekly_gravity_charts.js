...
function renderChart(ticker, tickerData, report) {
  ...
  parts.push(
    ...
    `<line x1="${PLOT_LEFT}" y1="${ZERO_Y}" x2="${PLOT_RIGHT}" y2="${ZERO_Y}" stroke="#e2e8f0" stroke-width="0.9"/>`,
    `<path d="${weightedPath}" fill="url(#gravityGradientY)" mask="url(#areaFadeMaskX)"/>`,
    `<path d="${weightedLine}" fill="none" stroke="url(#curveGradientX)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>`,
    `<path d="${netLine}" fill="none" stroke="#94a3b8" stroke-width="1.1" stroke-dasharray="3 3" stroke-linecap="round" stroke-linejoin="round" opacity="0.65"/>`,
    renderLogoWatermark(),
    `<line x1="${spotX}" y1="${PLOT_TOP - 4}" x2="${spotX}" y2="${PLOT_BOTTOM + 24}" stroke="${SPOT_COLOR}" stroke-width="1.3" stroke-dasharray="5 5"/>`,
