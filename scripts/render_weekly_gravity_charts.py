...
def render_chart(ticker, ticker_data, report):
    ...
    parts.extend([
        ...
        f'<line x1="{PLOT_LEFT}" y1="{ZERO_Y}" x2="{PLOT_RIGHT}" y2="{ZERO_Y}" stroke="#e2e8f0" stroke-width="0.9"/>',
        f'<path d="{weighted_path}" fill="url(#gravityGradientY)" mask="url(#areaFadeMaskX)"/>',
        f'<path d="{weighted_line}" fill="none" stroke="url(#curveGradientX)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>',
        f'<path d="{net_line}" fill="none" stroke="#94a3b8" stroke-width="1.1" stroke-dasharray="3 3" stroke-linecap="round" stroke-linejoin="round" opacity="0.65"/>',
        render_logo_watermark(),
        f'<line x1="{spot_x}" y1="{PLOT_TOP - 4}" x2="{spot_x}" y2="{PLOT_BOTTOM + 24}" stroke="{SPOT_COLOR}" stroke-width="1.3" stroke-dasharray="5 5"/>',
